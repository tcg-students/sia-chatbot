...
export const getInitialTreeText = (_) => {
  return async (dispatch) => {
    const response = await axios.get("http://localhost:4000/treeMessages");
    const getData = response.data;
    console.log("getData", getData);
    dispatch({ type: actions.GET_TREE_MESSAGES, payload: getData });
  };
};
...
export const getInitialNode = (id) => {
  return async (dispatch) => {
    ...
    dispatch({ type: actions.GET_TREE_MESSAGES, payload: getInitialNodeData });
  };
};
